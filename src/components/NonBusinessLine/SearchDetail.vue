      ...
      methods: {
        loadMore(index){
          ...
          if(this.count>this.DayPlanDetail.length&&index===this.showmum){
            var url='http://whjjgc.r93535.com/NonBusinessDayPlanDetailSearchServlet?xmmc='+this.xmmc+'&sgksrq='+this.currenttime+'&page='+this.page+'&baseuserid='+this.baseuserid+'&sgjsrq='+this.currenttime
        ...
        getCount(data) {
          ...
          var url='http://whjjgc.r93535.com/DayPlanCountServlet?baseuserid='+this.baseuserid+'&xmmc='+this.xmmc+'&type=3&sgksrq='+this.sgksrq+'&sgjsrq='+this.sgjsrq+'&dj='+this.dj+'&xingbie='+this.xingbie+'&sglx='+this.sglx+'&sglc=&kssjd=&page=0'
          axios.get(url)
            .then(response => {
              debugger
        ...
        getDayPlanDetail(data,index,flag) {
          if(this.hasopen==false&&flag==true){
            ...
            var url='http://whjjgc.r93535.com/NonBusinessDayPlanDetailSearchServlet?xmmc='+this.xmmc+'&sgksrq='+this.currenttime+'&page='+this.page+'&baseuserid='+this.baseuserid+'&sgjsrq='+this.currenttime
            //var url='http://whjjgc.r93535.com/DayPlanDetailNearbySearchServlet?xmmc='+this.xmmc+'&sgksrq='+data+'&page=1&sgjsrq='+data+'&baseuserid=236000&xb='+this.xingbie+'&lb='+this.sglx
        ...
        onClickLeft() {
          this.$router.push({path: '/NonBusinessLine/Search'});
          Toast('返回');
        },
        onClickRight() {
          this.$router.push({path: '/NonBusinessLine/Search'});

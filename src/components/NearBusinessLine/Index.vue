...
//      this.$store.commit('setInfiniteLoading',{infiniteLoading:this.infiniteLoading });
    ...
    methods:{
      ...
      getList(){
        let vm = this;
        let url = 'http://tljjgxt.r93535.com/DayPlanDetailNearbyServlet?page='+vm.page+'&baseuserid='+vm.baseuserid+'&sgrq='+vm.sgrq+'&xmmc='+vm.xmmcId;
      ...
      onInfinite(done) {
        let vm = this;
        vm.counter++;
        let url ='http://tljjgxt.r93535.com/DayPlanDetailNearbyServlet?page='+ vm.counter +'&baseuserid='+this.baseuserid+'&sgrq='+vm.sgrq;
        vm.$http.get(url).then((response) => {
      ...
      goSearchPage(date,slectProjectId,selectProjectName){
        this.$router.push({path: '/NearBusinessLine/NearBusinessLineSearch',query:{date:date.join('-'),slectProjectId:slectProjectId,slectProjectName:selectProjectName}});
      },
